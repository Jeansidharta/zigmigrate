...
fn runUp() !void {
    // read the json file
    var parsed = try zigmigrate.allocJsonFromFile(zigmigrate.Config, allocator, "zigmigrate.json");
    defer parsed.deinit();
    ...
    var driver = zigmigrate.Driver{ .sqlite_driver = &sqlite_driver };
...
fn runInit() !void {
    ...
    const configuration = zigmigrate.Config{
...
fn runCreateMigration() !void {
    ...
    const filename = try zigmigrate.generateMigrationFileName(allocator, migration_name);
